...
export default function PoolPage({ protocol, pair }: PoolPageProps) {
  ...
  const poolData = useMemo(() => {
    if (!token0Name || !token1Name) return undefined;

    try {
      const foundPair = pairs.find((pr) => {
        if (!pr?.token0Details?.name || !pr?.token1Details?.name) return false;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setChartError(null);
        const to = Math.floor(currentTime / 1000);
        let from: number;
        switch (period) {
          case "24h":
            from = to - 24 * 3600;
    ...
    void loadData();
  }, [poolData, period, fetchCandles, token0Name, token1Name, currentTime]);

  // Conditional returns after all hooks
  if (!protocolParam || !pairParam) {
    return (
      <ErrorState
        ...
        <header className="py-4 border-b border-border">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <Badge
                  ...
                  className={cn(
                    ...
                    protocol === "soroswap" && "bg-purple-500/10 text-purple-500 border-purple-500/20",
                    protocol === "blend" && "bg-green-500/10 text-green-500 border-green-500/20",
                    protocol === "phoenix" && "bg-orange-500/10 text-orange-500 border-orange-500/20",
                    protocol === "aqua" && "bg-pink-500/10 text-pink-500 border-pink-500/20"
                  )}
                  onClick={() => {
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="default" size="sm" className="gap-2" onClick={() => window.open(`https://app.${protocol}.finance/pool/${poolData.market}`, "_blank")}>
                ...
                <Card className="border-none shadow-md">
                  ...
                  <CardContent className="p-6">
                    <div className="space-y-6">
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        <StatCard title="Protocol Version" value={`${getProtocolDisplay(protocol)} V1`} tooltip="Current protocol version" icon={<Tag className="h-4 w-4 text-primary" />} />
