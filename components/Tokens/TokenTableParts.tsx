 ...
 */
export function TokenTableRow({ token, pairMap, volumeMap, tvlMap, isServer = false }: TokenTableRowProps) {
    const [symbolName] = token.name.split(":")
    const symbolIssuer = token.name === "native" ? "XLM" : `${token.name.substring(0, 8)}...${token.name.substring(token.name.length - 4)}`
// Price
...
const priceDisplay =
    ...
    const firstCellContent = isServer ? (
        <a href={detailsUrl} className="tokenSymbolContainer ">
            <div className="tokenSymbolLabel">
    ...
    return (
        <tr
            ...
            {...rowClickProps}
        >
            <td role="cell" className="text-left TokenTableCell">{firstCellContent}</td>
    ...
    if (tokens.length === 0) {
        return (
            <tbody>
                <tr role="row">
                    <td
                        colSpan={noTokensColSpan}
                        className="noTokensFoundCell"
                        role="cell"
                    >
                        No tokens found
                    </td>
